...
import { addDays, addWeeks, addMonths, startOfDay, getDay, isBefore, isEqual, isAfter, subMonths, addYears, format } from 'date-fns';
...
export default function AddProductScreen() {
  const { user, addProduct, updateInventory, addScheduledDoses } = useApp();
  const [name, setName] = useState('');
  const [category, setCategory] = useState('');
  const [medicationType, setMedicationType] = useState<MedicationType>('Other Peptide');
  const [doseMg, setDoseMg] = useState('');
  const [selectedFrequencies, setSelectedFrequencies] = useState<Frequency[]>(['Daily']);
  ...
  const calculateScheduledDoses = (productId: string, productName: string, doseAmount: number, routeType: Route): ScheduledDose[] => {
    ...
    // This ensures we're working with consistent date boundaries
    ...
    console.log('=== CALCULATING SCHEDULED DOSES ===');
    ...
    selectedFrequencies.forEach(frequency => {
      ...
      if (frequency === 'AM Daily' || frequency === 'PM Daily' || frequency === 'Daily' || 
        ...
        // Fixed interval frequencies - start from the effective start date (inclusive)
        ...
        // Use <= for the end date to be inclusive
        while (isBefore(currentDate, endDate) || isEqual(currentDate, endDate)) {
          // Format date as YYYY-MM-DD in local timezone
          const dateString = format(currentDate, 'yyyy-MM-dd');
          
          doses.push({
            id: `${productId}-${frequency}-${dateString}-${scheduledTime}`,
            productId,
            productName,
            doseMg: doseAmount,
            route: routeType,
        ...
        if (doseCount > 0) {
          console.log(`First dose: ${doses[doses.length - doseCount].scheduledDate}`);
        }
        ...
        if (daysOfWeek.length > 0) {
          ...
          selectedDayNumbers.forEach(targetDay => {
            ...
            while (isBefore(currentDate, endDate) || isEqual(currentDate, endDate)) {
              ...
              doses.push({
                id: `${productId}-${frequency}-${targetDay}-${dateString}-${scheduledTime}`,
                productId,
                productName,
                doseMg: doseAmount,
                route: routeType,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      addScheduledDoses(scheduledDoses);
